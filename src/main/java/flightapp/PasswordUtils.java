...
public class PasswordUtils {
  ...
  public static byte[] saltAndHashPassword(String password) {
    ...
    // Create the combined signle byte array
    byte[] saltedPassword = new byte[salt.length + saltedHash.length];
    ...
    for (int j = 0; j < saltedHash.length; j++) {
      saltedPassword[salt.length + j] = saltedHash[j];
    }
  ...
  public static boolean plaintextMatchesSaltedHash(String plaintext, byte[] saltedHashed) {
    ...
    byte[] passwordHased = new byte[saltedHashed.length - SALT_LENGTH_BYTES];;
    ...
    for (int j = 0; j < saltedHashed.length - SALT_LENGTH_BYTES; j++) {
      passwordHased[j] = saltedHashed[SALT_LENGTH_BYTES + j];
    }

    return Arrays.equals(hashWithSalt(plaintext, salt), passwordHased);
  }
